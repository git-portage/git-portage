...
# $Header: /var/cvsroot/gentoo-x86/dev-ruby/bundler/Attic/bundler-1.0.8.ebuild,v 1.1 2011/01/19 20:20:37 graaff Exp $
...
all_ruby_prepare() {
	...
	sed -i -e '626s/should/should_not/' spec/runtime/setup_spec.rb || die
	...
	# Remove focus on a single spec to we can run them all
	sed -i -e 's/fit/it/' spec/install/gems/packed_spec.rb || die
}
