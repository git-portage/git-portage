...
# $Header: /var/cvsroot/gentoo-x86/dev-ruby/yard/Attic/yard-0.7.1.ebuild,v 1.2 2011/08/07 14:10:53 armin76 Exp $
...
all_ruby_prepare() {
	# Patch out a spec that depends on a specific LANG setting.
	sed -i -e '41s/RUBY18/false/' spec/templates/helpers/html_helper_spec.rb || die
}
...
each_ruby_prepare() {
	case ${RUBY} in
		*jruby)
			...
			sed -i -e '122s/should/should_not/' spec/templates/helpers/html_helper_spec.rb || die
			;;
		*)
			;;
	esac
