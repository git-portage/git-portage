...
# $Header: /var/cvsroot/gentoo-x86/games-puzzle/pouetchess/Attic/pouetchess-0.2.0.ebuild,v 1.1 2006/06/14 18:19:29 wolf31o2 Exp $
...
src_unpack() {
	...
	epatch "${FILESDIR}"/${P}-sconstruct-sandbox.patch || die "patch failed"

	# Fix up nvidia headers
	epatch "${FILESDIR}"/${P}-nvidia_glext.patch

	# Fix for LibSDL >= 1.2.10 detection
	sed -i -e "s:sdlver.split('.') >= \['1','2','8'\]:sdlver.split('.') >= [1,2,8]:" pouetChess.py || die "sed failed"
}
...
src_install() {
	# usings scons to install will violate sandbox
	...
	dodoc ChangeLog README || die "installing docs failed"

	doicon data/icons/${MY_PN}.png || die "installing icon failed"
	make_desktop_entry ${MY_PN} "pouetChess" ${MY_PN}.png "KDE;Qt;Game;BoardGame" || die "creating desktop entry failed"
