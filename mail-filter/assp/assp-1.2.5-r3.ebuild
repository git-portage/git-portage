...
# $Header: /var/cvsroot/gentoo-x86/mail-filter/assp/Attic/assp-1.2.5-r3.ebuild,v 1.1 2006/11/20 22:48:33 wltjr Exp $
...
src_unpack() {
	...
	sed -i -e "/foreach /s:('.':('/var/lib/assp/':" \
		...
		-e 's:glob("$base/$logdir*$logdirfile");:glob("$logdir*$logdirfile");:' \
		...
		-e 's:"$base/$maillog/:"/var/lib/assp/$maillog/:' \
		...
		-e 's:if ($fil=~/\.\./) {:if (($fil=~/\.\./) or ($fil!~/^images\/.*/)):' \
	...
	sed -i -e 's:tail -300 -f /usr/local/assp/maillog.log:tail -n 300 -f /var/log/assp/maillog.txt:' \
		-e 's/\r//' \
		"${S}/stats.sh" || die "sed failed"

	# just being safe, same sed stuff called on all modified files above
	edos2unix "${S}/freshclam.sh"
	edos2unix "${S}/repair.pl"
	edos2unix "${S}/stat.pl"

	# rename utils
	mv "${S}/freshclam.sh" "${S}/asspfreshclam.sh"
