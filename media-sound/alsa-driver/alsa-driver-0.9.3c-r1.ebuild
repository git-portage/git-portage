...
# $Header: /var/cvsroot/gentoo-x86/media-sound/alsa-driver/Attic/alsa-driver-0.9.3c-r1.ebuild,v 1.3 2003/09/07 00:06:04 msterret Exp $
...
src_install() {
	...
	# The "install" target may attempt to create device entries in /dev.
	# We don't want that to happen, so we bypass it by calling the install-*
	# targets explicitely.
	make DESTDIR=${D} \
	    install-modules install-headers install-scripts check-snd-prefix || die
