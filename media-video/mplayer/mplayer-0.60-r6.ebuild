...
# $Header: /var/cvsroot/gentoo-x86/media-video/mplayer/Attic/mplayer-0.60-r6.ebuild,v 1.1 2002/04/17 11:33:33 seemant Exp $
...
MY_P="${MY_PN}-${MY_PV}"
S="${WORKDIR}/${MY_PN}"
# Only install Skin if GUI should be built (gtk in USE)
SRC_URI="http://www.mplayerhq.hu/MPlayer/cvs/${MY_P}.tar.bz2
...
src_unpack() {
	...
	unpack ${MY_P}.tar.bz2 mp-arial-iso-8859-1.zip
...
src_compile() {
	...
	( use mmx || use 3dnow || use sse ) && ( \
		myconf="${myconf} --disable-runtime-cpudetection"
	) || ( \
		myconf="${myconf} --disable-mmx --disable-mmx2"
		myconf="${myconf} --disable-fastmemcpy"
	)
	...
	use dvd \
		&& myconf="${myconf} --enable-dvdread --enable-css --enable-dvdnav"
	...
	use nas \
		|| myconf="${myconf} --disable-straming"
