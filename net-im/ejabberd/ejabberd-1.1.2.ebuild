...
# $Header: /var/cvsroot/gentoo-x86/net-im/ejabberd/Attic/ejabberd-1.1.2.ebuild,v 1.1 2006/10/13 15:54:02 chainsaw Exp $
...
SRC_URI="http://process-one.net/en/projects/${PN}/download/${PV}/${P}.tar.gz mirror://gentoo/ejabberd-patchball-${PV}.tar.bz2
	 mod_presence? ( http://linuxlovers.at/site/files/${P}-presence-pixmaps.tar.gz )"
LICENSE="GPL-2"
SLOT="0"
KEYWORDS="~x86"
...
pkg_postinst() {
	...
	if useq socks5 ; then
		einfo "Info about the SOCKS5 proxy support can be found at http://ejabberd.jabber.ru/mod_proxy65"
	fi
	if useq mod_presence ; then
		einfo "Info about the presence module can be found at http://ejabberd.jabber.ru/mod_presence"
	fi
	if useq odbc ; then
