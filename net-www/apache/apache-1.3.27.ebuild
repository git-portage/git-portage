...
# $Header: /var/cvsroot/gentoo-x86/net-www/apache/Attic/apache-1.3.27.ebuild,v 1.1 2002/10/06 09:37:34 carpaski Exp $
...
src_unpack() {
	...
	# some nice patches..
	patch -p1 < ${WORKDIR}/${PV}/mdk/apache_1.3.11-apxs.patch || die m1
	patch -p1 < ${WORKDIR}/${PV}/mdk/apache_1.3.22-srvroot.patch || die m2
	patch -p1 < ${WORKDIR}/${PV}/mdk/apache-1.3.14-mkstemp.patch || die m3
	patch -p0 < ${WORKDIR}/${PV}/mdk/apache-1.3.20.manpage.patch || die m4
	patch -p1 < ${WORKDIR}/${PV}/deb/apxs_wrong_prefix || die d1
	patch -p1 < ${WORKDIR}/${PV}/deb/custom_response_segfaults || die d2
	patch -p1 < ${WORKDIR}/${PV}/deb/mime_type_fix || die d3
	patch -p1 < ${WORKDIR}/${PV}/deb/regex_must_conform_to_posix_for_LFS_to_work || die d4
	patch -p1 < ${WORKDIR}/${PV}/deb/suexec_combined || die d5
	patch -p1 < ${WORKDIR}/${PV}/deb/suexec_of_death || die d6
	patch -p1 < ${WORKDIR}/${PV}/deb/usr_bin_perl_owns_you || die d7
