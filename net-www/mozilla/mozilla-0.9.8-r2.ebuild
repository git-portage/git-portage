...
# $Header: /var/cvsroot/gentoo-x86/net-www/mozilla/Attic/mozilla-0.9.8-r2.ebuild,v 1.2 2002/04/12 19:02:37 spider Exp $
...
pkg_postinst() {
	...
	if [ -e /usr/lib/mozilla/component.reg ] ; then
	...
	if [ -e /usr/lib/mozilla/components/libtimer_gtk.so ] ; then
		rm -f /usr/lib/mozilla/components/libtimer_gtk.so
	fi
...
pkg_postrm() {
	...
	if [ -e /usr/lib/mozilla/regxpcom ] ; then
