...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/Attic/portage-2.1.1_pre3-r2.ebuild,v 1.1 2006/07/20 21:01:08 zmedico Exp $
...
src_unpack() {
	...
	if [ -n "${PATCHVER}" ]; then
		...
		epatch "${FILESDIR}/2.1.1_pre3-r2-fixes.patch"
	fi
	if [ "${PR}" != "r0" ]; then
