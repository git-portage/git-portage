...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/Attic/portage-2.1_rc4-r2.ebuild,v 1.1 2006/06/05 08:10:19 zmedico Exp $
...
src_unpack() {
	...
	if [ "${PR}" != "r0" ]; then
		...
		epatch "${FILESDIR}/repoman_livevcs_fixes.patch"
...
src_install() {
	...
	if use linguas_pl; then
		for x in pl pl.UTF-8; do
			doman -i18n=${x} "${S_PL}"/man/${x}/*.[0-9]
		done
	fi
	dodoc "${S}"/ChangeLog
	dodoc "${S}"/NEWS
