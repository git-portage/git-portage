...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/e2fsprogs/Attic/e2fsprogs-1.36-r1.ebuild,v 1.1 2005/03/09 00:35:52 vapier Exp $
...
src_install() {
	...
	# install linker scripts to /usr/lib/, and fix the .so symlinks in /lib/
	dodir /$(get_libdir)
	...
	for x in *.so ; do
		...
		dosym ${x}.? /$(get_libdir)/${x} || die "dosym ${x}"
	done

	# move 'useless' stuff to /usr/
	dosbin "${D}"/sbin/mklost+found
	rm -f "${D}"/sbin/mklost+found
	# convert hard links to sym links
	rm -f "${D}"/sbin/{fsck,mkfs}.*
	dosym e2fsck /sbin/fsck.ext2
	dosym e2fsck /sbin/fsck.ext3
	dosym mke2fs /sbin/mkfs.ext2
	dosym mke2fs /sbin/mkfs.ext3
