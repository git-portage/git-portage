...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/lvm2/Attic/lvm2-2.02.63-r2.ebuild,v 1.2 2010/05/07 18:22:16 robbat2 Exp $
...
src_install() {
	...
	dodir /$(get_libdir)
	# .so -> /$(get_libdir)
	mv -f "${D}"/usr/lib/lib*.so* "${D}"/$(get_libdir)
	[[ "$(get_libdir)" != "lib" ]] && \
		mv "${D}"/usr/$(get_libdir)/lib*.so* "${D}"/$(get_libdir)
	# .a -> /usr/$(get_libdir)
	[[ "$(get_libdir)" != "lib" ]] && \
		mv -f "${D}"/usr/lib/lib*.a "${D}"/usr/$(get_libdir)
	# The upstream symlinks are borked. lets rebuild them instead.
	find "${D}"/{usr,}/{lib,$(get_libdir)} -type l \
		| xargs rm -f 2>/dev/null
	for i in "${D}"/$(get_libdir)/*.so.* ; do
		b="${i//*\/}" o="${b/.so.*/.so}"
		ln -s "${b}" "${D}/$(get_libdir)/${o}"
	done
	# Now enable building properly
	for i in \
