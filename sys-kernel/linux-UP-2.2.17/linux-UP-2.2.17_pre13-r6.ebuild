...
# $Header: /var/cvsroot/gentoo-x86/sys-kernel/linux-UP-2.2.17/Attic/linux-UP-2.2.17_pre13-r6.ebuild,v 1.3 2000/09/15 20:09:27 drobbins Exp $

P=linux-UP-2.2.17p13
A="linux-2.2.16.tar.bz2 i2c-2.5.2.tar.gz lm_sensors-2.5.2.tar.gz
   linux-2.2.16-reiserfs-3.5.24-patch.gz pre-patch-2.2.17-13.bz2
...
SRC_URI="ftp://ftp.uk.kernel.org/pub/linux/kernel/v2.2/linux-2.2.16.tar.bz2
	 ...
	 http://devlinux.com/pub/namesys/linux-2.2.16-reiserfs-3.5.24-patch.gz
	 ...
	 http://www.fys.uio.no/~trondmy/src/linux-2.2.17pre13-nfsv3-0.22.3.dif.bz2
...
#    unpack ${A5}
    ...
    bzip2 -dc ${DISTDIR}/linux-2.2.17pre13-nfsv3-0.22.3.dif.bz2 | patch -p1
    ...
    gzip -dc ${DISTDIR}/linux-2.2.16-reiserfs-3.5.24-patch.gz | patch -p1
...
##    sed -e 's:-O2:${CFLAGS}:g' Makefile.orig > Makefile
    ...
    try make symlinks
    cp ${O}/files/${P}-r6.config .config
    cp ${O}/files/${P}-r6.autoconf include/linux/autoconf.h
