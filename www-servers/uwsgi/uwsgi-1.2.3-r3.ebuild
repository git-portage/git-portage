...
# $Header: /var/cvsroot/gentoo-x86/www-servers/uwsgi/Attic/uwsgi-1.2.3-r3.ebuild,v 1.1 2012/05/24 10:31:48 dev-zero Exp $
...
src_compile() {
	...
	if use python ; then
		for a in ${PYTHON_ABIS} ; do
			...
			use gevent && python${a} uwsgiconfig.py --plugin plugins/gevent gentoo gevent${a/.} || die "building plugin for gevent-support in python-${a} failed"
			if use erlang ; then
