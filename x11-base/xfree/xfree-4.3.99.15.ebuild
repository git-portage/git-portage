...
# $Header: /var/cvsroot/gentoo-x86/x11-base/xfree/Attic/xfree-4.3.99.15.ebuild,v 1.1 2003/11/02 22:05:55 spyderous Exp $
...
# portage for USE_EXPAND
# ghostscript for PDF docs, which aren't even working yet
...
src_unpack() {
	...
	if use doc
	then
		echo "#define BuildLinuxDocText YES" >> config/cf/host.def
		echo "#define BuildLinuxDocHtml YES" >> config/cf/host.def
		echo "#define BuildLinuxDocPS YES" >> config/cf/host.def
		echo "#define BuildSpecsDocs YES" >> config/cf/host.def
	else
